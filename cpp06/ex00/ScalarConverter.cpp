...
static void check_dot_float(const std::string& rhs, bool& dot_flag,
                            bool& float_flag) {
  for (std::size_t i = 0; i < rhs.size(); i++) {
    if (!i && rhs[i] == '-') continue;
    if (!(std::isdigit(rhs[i]) || rhs[i] == '.' || rhs[i] == 'f') ||
        (dot_flag && rhs[i] == '.'))
      throw 0;
    if (rhs[i] == '.') dot_flag = true;
    if (rhs[i] == 'f' && i != rhs.size() - 1) throw 0;
  }
  if (rhs[rhs.size() - 1] == '.') throw 0;
  if (rhs[0] == '.' || (rhs.size() >= 2 && rhs[0] == '-' && rhs[1] == '.')) throw 0;
  if (rhs[rhs.size() - 1] == 'f') float_flag = true;
}
...
static void convert_by_double(const std::string& rhs) {
  ...
  if (errno == ERANGE && (d == HUGE_VAL || d == 0)) throw 0;
  if (d > std::numeric_limits<char>::max() ||
      d < std::numeric_limits<char>::min() || d != std::floor(d))
  ...
  if (d > std::numeric_limits<int>::max() ||
      d < std::numeric_limits<int>::min() || d != std::floor(d))
  ...
  if (d > std::numeric_limits<float>::max() ||
    ...
    if (f != std::floor(f))
      std::cout << std::fixed << "float: " << std::setprecision(rhs.size() - rhs.find('.') - 1) << f << "f\n";
    else
      std::cout << std::fixed << "float: " << std::setprecision(1) << f << "f\n";
  }
  if (d != std::floor(d))
    std::cout << std::fixed << "double: " << std::setprecision(rhs.size() - rhs.find('.') - 1) << d << '\n';
  else
    std::cout << std::fixed << "double: " << std::setprecision(1) << d << "\n";
}
...
static void convert_by_float(const std::string& rhs) {
  ...
  if (errno == ERANGE && (f == HUGE_VALF || f == 0)) throw 0;
  d = static_cast<double>(f);
  if (f > std::numeric_limits<char>::max() ||
      f < std::numeric_limits<char>::min() || f != std::floor(f))
    std::cout << "char: impossible\n";
  ...
  if (f != std::floor(f))
    std::cout << std::fixed << "float: " << std::setprecision(rhs.size() - rhs.find('.') - 2) << f << "f\n";
  else
    std::cout << std::fixed << "float: " << std::setprecision(1) << f << "f\n";
  if (d != std::floor(d))
    std::cout << std::fixed << "double: " << std::setprecision(rhs.size() - rhs.find('.') - 2) << d << '\n';
  else
    std::cout << std::fixed << "double: " << std::setprecision(1) << d << "\n";
}
